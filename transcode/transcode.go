...
var (
	MP3   = NewProfile("audio/mpeg", "mp3", 128, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libmp3lame -f mp3 <metadata> -`)
	MP3RG = NewProfile("audio/mpeg", "mp3", 128, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libmp3lame -af "volume=replaygain=track:replaygain_preamp=6dB:replaygain_noclip=0, alimiter=level=disabled, asidedata=mode=delete:type=REPLAYGAIN" -metadata replaygain_album_gain= -metadata replaygain_album_peak= -metadata replaygain_track_gain= -metadata replaygain_track_peak= -metadata r128_album_gain= -metadata r128_track_gain= -f mp3 <metadata> -`)

	Opus       = NewProfile("audio/ogg", "opus", 96, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libopus -vbr on -f opus -`)
	OpusRG     = NewProfile("audio/ogg", "opus", 96, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libopus -vbr on -af "volume=replaygain=track:replaygain_preamp=6dB:replaygain_noclip=0, alimiter=level=disabled, asidedata=mode=delete:type=REPLAYGAIN" -metadata replaygain_album_gain= -metadata replaygain_album_peak= -metadata replaygain_track_gain= -metadata replaygain_track_peak= -metadata r128_album_gain= -metadata r128_track_gain= -f opus <metadata> -`)
	OpusRGLoud = NewProfile("audio/ogg", "opus", 96, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libopus -vbr on -af "aresample=96000:resampler=soxr, volume=replaygain=track:replaygain_preamp=15dB:replaygain_noclip=0, alimiter=level=disabled, asidedata=mode=delete:type=REPLAYGAIN" -metadata replaygain_album_gain= -metadata replaygain_album_peak= -metadata replaygain_track_gain= -metadata replaygain_track_peak= -metadata r128_album_gain= -metadata r128_track_gain= -f opus <metadata> -`)

	Opus128       = NewProfile("audio/ogg", "opus", 128, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libopus -vbr on -f opus <metadata> -`)
	Opus128RG     = NewProfile("audio/ogg", "opus", 128, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libopus -vbr on -af "volume=replaygain=track:replaygain_preamp=6dB:replaygain_noclip=0, alimiter=level=disabled, asidedata=mode=delete:type=REPLAYGAIN" -metadata replaygain_album_gain= -metadata replaygain_album_peak= -metadata replaygain_track_gain= -metadata replaygain_track_peak= -metadata r128_album_gain= -metadata r128_track_gain= -f opus <metadata> -`)
	Opus128RGLoud = NewProfile("audio/ogg", "opus", 128, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -b:a <bitrate> -c:a libopus -vbr on -af "aresample=96000:resampler=soxr, volume=replaygain=track:replaygain_preamp=15dB:replaygain_noclip=0, alimiter=level=disabled, asidedata=mode=delete:type=REPLAYGAIN" -metadata replaygain_album_gain= -metadata replaygain_album_peak= -metadata replaygain_track_gain= -metadata replaygain_track_peak= -metadata r128_album_gain= -metadata r128_track_gain= -f opus <metadata> -`)

	Raw = NewProfile("", "", 0, `ffmpeg -v 0 <seek> -i <file> -map 0:a:0 -vn -c:a copy <metadata> -`)
)
...
func (p *Profile) SetBitrate(bitRate BitRate) {
	p.bitrate = bitRate
}

func (p *Profile) SetSeek(seek time.Duration) {
	p.seek = seek
}

func (p *Profile) SetSource(suffix string) {
	p.suffix = suffix
	p.mime = mime.TypeByAudioExtension(suffix)
}

func (p *Profile) SetMetadata(metadata map[string]string) {
	p.metadata = metadata
}

func (p *Profile) SetInterval(start, duration time.Duration) {
	p.seek = start
	p.duration = duration
}
