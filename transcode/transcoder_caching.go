...
func NewCachingTranscoder(t Transcoder, c *cache.DirCache) *CachingTranscoder {
	return &CachingTranscoder{transcoder: t, cache: c}
}

func (t *CachingTranscoder) Transcode(ctx context.Context, profile Profile, in string, out io.Writer) error {
	t.cache.RLock()
	defer t.cache.RUnlock()
	...
	if err := os.MkdirAll(t.cache.Path(), perm^0o111); err != nil {
