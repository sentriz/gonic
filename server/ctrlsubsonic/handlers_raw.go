...
func (c *Controller) recordCoverPreference(userID int, client string, size int) {
	pref := db.ClientCoverSizePreference{UserID: userID, Client: client}
	if err := c.dbc.
		Where("user_id=? AND client=?", userID, client).
		Assign(db.ClientCoverSizePreference{LastCoverSize: size}).
